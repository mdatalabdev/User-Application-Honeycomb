...
from Notifications.db_notification.crud import get_notifications, get_last_notification_timestamp, close_notification
...
@app.post(
    "/downlink/notifications/{notification_id}/close",
...
async def get_new_notifications(
    ...
    try:
        ...
        logging.error(f"Failed to fetch NEW notifications: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to fetch notifications")
    
@app.get(
    "/downlink/notifications/closed",
    summary="Get CLOSED notifications"
)
async def get_closed_notifications(
    limit: int = 50,
    offset: int = 0,
    db: Session = Depends(get_db),
    current_user=Depends(auth.get_current_user)
    ...
    try:
        ...
        logging.error(f"Failed to fetch CLOSED notifications: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to fetch notifications")